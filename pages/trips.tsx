  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-6xl">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
        ...
        <Button onClick={() => router.push("/plan")} className="shrink-0">Plan New Trip</Button>
      </div>
      ...
      {isLoading ? (
            ...
            <Card key={trip.id} className="group hover:border-primary/50 transition-colors flex flex-col h-full overflow-hidden hover-elevate">
              ...
              <CardHeader>
                <div className="flex justify-between items-start">
                  ...
                  <span className="text-xs bg-muted px-2 py-1 rounded-full whitespace-nowrap capitalize border">{trip.travelStyle}</span>
                </div>
                <div className="text-xs text-muted-foreground mt-1">Created {format(new Date(trip.createdAt), "MMM d, yyyy")}</div>
              </CardHeader>
              <CardContent className="flex-1 space-y-4">
                ...
                <p className="text-sm text-muted-foreground line-clamp-3">{trip.plan.summary}</p>
              </CardContent>
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    ...
                    <AlertDialogFooter>
                      ...
                      <AlertDialogAction onClick={() => handleDelete(trip.id)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">Delete</AlertDialogAction>
                    </AlertDialogFooter>
                  </AlertDialogContent>
                </AlertDialog>
                <Button variant="secondary" onClick={() => router.push(`/trips/${trip.id}`)}>
        ...
        <div className="text-center py-24 bg-card border rounded-2xl border-dashed">
          ...
          <p className="text-muted-foreground max-w-md mx-auto mb-8">Your travel library is empty. Let our AI craft the perfect itinerary for your next adventure.</p>
          <Button onClick={() => router.push("/plan")} size="lg">Start Planning</Button>
        </div>
      )}
    </div>
  );
