...
import { useGetTrip, useDeleteTrip, getListTripsQueryKey, getGetTripQueryKey } from "@/lib/api-client";
...
import { Calendar, Hotel, Train, Backpack, DollarSign, ArrowLeft, Trash2, Map, Sunrise, Cloud } from "lucide-react";
...
export default function TripDetail() {
  ...
  const { data: trip, isLoading, isError } = useGetTrip(tripId, {
    query: { enabled: !!tripId && !isNaN(tripId), queryKey: getGetTripQueryKey(tripId) },
  });
  ...
  if (isError || !trip) {
    return (
      <div className="container mx-auto px-4 py-24 text-center">
        ...
        <Button onClick={() => router.push("/trips")}>Return to My Trips</Button>
      </div>
    );
  }
    ...
    <div className="container mx-auto px-4 py-8 max-w-6xl pb-24">
        ...
        <AlertDialog>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>This will permanently delete your trip plan to {trip.destination}.</AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
      ...
      <div className="relative overflow-hidden rounded-3xl bg-primary text-primary-foreground mb-8 p-8 md:p-12 shadow-xl">
        <div className="relative z-10">
          <div className="flex flex-wrap items-center gap-3 mb-4">
            <span className="bg-primary-foreground/20 text-primary-foreground px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wider">{trip.travelStyle}</span>
            <span className="bg-primary-foreground/20 text-primary-foreground px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wider">{trip.days} Days</span>
          </div>
          <h1 className="text-4xl md:text-6xl font-serif font-bold tracking-tight mb-4">{trip.destination}</h1>
          <p className="text-lg md:text-xl text-primary-foreground/80 max-w-3xl leading-relaxed">{tripPlan.summary}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div key={idx} className="relative">
                  <div className="sticky top-20 bg-background/95 backdrop-blur z-10 py-4 mb-4 border-b">
                    <h3 className="font-bold text-2xl font-serif flex items-center gap-3">
                      <span className="flex items-center justify-center w-10 h-10 rounded-full bg-primary text-primary-foreground text-base">{day.day}</span>
                      {day.title}
                    </h3>
                  </div>
                      ...
                      <div key={aIdx} className="group relative bg-card border rounded-xl p-5 hover-elevate transition-all">
                        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-3">
                          ...
                          {activity.estimatedCost && (
                            <span className="shrink-0 bg-muted px-3 py-1 rounded-md text-sm font-semibold border">${activity.estimatedCost}</span>
                          )}
                        </div>
        ...
        <div className="space-y-6">
            ...
            <CardContent className="pt-6 space-y-4">
              {tripPlan.bestTimeToVisit && (
                <div className="flex gap-4">
                  ...
                  <div><p className="font-semibold">Best Time to Visit</p><p className="text-sm text-muted-foreground mt-1">{tripPlan.bestTimeToVisit}</p></div>
                </div>
              )}
              {tripPlan.weather && (
                <div className="flex gap-4">
                  <div className="bg-background p-2 rounded-lg h-fit border shadow-sm"><Cloud className="w-5 h-5 text-primary" /></div>
                  <div><p className="font-semibold">Typical Weather</p><p className="text-sm text-muted-foreground mt-1">{tripPlan.weather}</p></div>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="bg-muted/30 border-b">
              <CardTitle className="text-xl font-serif flex items-center justify-between">
                <span>Budget</span><span className="text-primary">${tripPlan.budgetBreakdown.total}</span>
              </CardTitle>
            ...
            <CardContent className="pt-6">
              <div className="h-48 mb-6">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    ...
                    ]} cx="50%" cy="50%" innerRadius={55} outerRadius={75} paddingAngle={2} dataKey="value">
              ...
              <div className="space-y-3 text-sm">
                {[["Accommodation", 0], ["Transport", 1], ["Food", 2], ["Activities", 3]].map(([label, idx]) => (
                  <div key={label as string} className="flex justify-between items-center">
                    <span className="flex items-center gap-2"><div className="w-3 h-3 rounded-full" style={{ backgroundColor: CHART_COLORS[idx as number] }}></div>{label}</span>
                    <span className="font-medium">${(tripPlan.budgetBreakdown as any)[(label as string).toLowerCase()]}</span>
                  </div>
                ))}
              </div>
            </CardContent>
          ...
          <div className="bg-card border rounded-xl overflow-hidden shadow-sm">
            <Tabs defaultValue="hotels" className="w-full">
              <TabsList className="w-full grid grid-cols-3 rounded-none border-b h-14 bg-muted/20">
                <TabsTrigger value="hotels" className="rounded-none data-[state=active]:bg-background data-[state=active]:border-b-2 data-[state=active]:border-b-primary"><Hotel className="w-4 h-4 mr-2" /> Stay</TabsTrigger>
                <TabsTrigger value="transport" className="rounded-none data-[state=active]:bg-background data-[state=active]:border-b-2 data-[state=active]:border-b-primary"><Train className="w-4 h-4 mr-2" /> Travel</TabsTrigger>
                <TabsTrigger value="packing" className="rounded-none data-[state=active]:bg-background data-[state=active]:border-b-2 data-[state=active]:border-b-primary"><Backpack className="w-4 h-4 mr-2" /> Pack</TabsTrigger>
              </TabsList>
              <div className="p-4 bg-background max-h-[600px] overflow-y-auto">
                    ...
                    <div key={idx} className="border-b last:border-0 pb-4 last:pb-0">
                      <div className="flex justify-between items-center mb-2">
                        <h4 className="font-bold capitalize flex items-center gap-2">
                          <div className="w-8 h-8 rounded-full bg-muted flex items-center justify-center"><Train className="w-4 h-4 text-primary" /></div>
                          {t.type}
                        </h4>
                    ...
                    <div key={idx}>
                      ...
                      <ul className="grid grid-cols-2 gap-2">
                        {category.items.map((item, i) => (
                          <li key={i} className="text-sm flex items-center gap-2 text-muted-foreground before:w-1.5 before:h-1.5 before:bg-muted-foreground/30 before:rounded-full">{item}</li>
                        ))}
                      </ul>
                    </div>
                  ))}
          ...
          <Card>
            <CardHeader><CardTitle className="text-lg font-serif flex items-center gap-2"><DollarSign className="w-5 h-5 text-primary" /> Saving Tips</CardTitle></CardHeader>
            <CardContent>
              <ul className="space-y-2">
                {tripPlan.budgetBreakdown.savingTips?.map((tip, i) => (
